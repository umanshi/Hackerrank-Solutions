...
int findShortest(int graph_nodes, vector<int> graph_from, vector<int> graph_to, vector<long> ids, int val) {
    ...
    vector<vector<long long> > adj ;// vector of vectors  PROPER
    for(int i=0; i<graph_nodes+1; i++)  // WAY TO FORM 
    {                                   // ADJACENCY
        vector<long long> v;            // LIST
        adj.push_back(v);
    }
    ...
    while(!q.empty())
    {
        ...
        cout<<"hahaha1"<<endl;
        cout<<"c-1 = "<<c-1<<endl;
        cout<<"ids[c-1] = "<<ids[c-1]<<endl;
        if(ids[c-1]==val && c!=s)
        {   found = 1;
            cout<<"same color found.. hahaha"<<endl;
            break;
        }
        cout<<"size = "<<adj[c].size()<<endl;
